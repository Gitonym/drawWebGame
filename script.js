window.addEventListener('load', (e) => {
    ...
    if (canvas.getContext) {
        ...
        drawTool.addEventListener('click', (e) => {changeTool(e, 'draw')});
        moveTool.addEventListener('click', (e) => {changeTool(e, 'move')});
        ...
        function accelPlayer(keyCode) {
            ...
            //movekeys
            if (pressed[87]) {
                if (player.onground &&( !isFree(player.x, player.y+1) || player.y + player.height  == 0)) {
                    //just jumped
        ...
        function isFree(x, y) {
            x -= camera.x;
            y -= camera.y;
            if (canvas.width < x || x < 0) {return false;}
            if (canvas.height < y || y < 0) {return false;}
            for (let yoffset = 0; yoffset < player.height; yoffset+=3) {
                for (let xoffset = 0; xoffset < player.width; xoffset+=3) {
                    if (ctx.getImageData(x + xoffset, y + yoffset, 1, 1).data[0] == 255) {
                        return false;
                    }
                }
            }
        ...
        function extendStroke(e) {
            if (!stroking) {return};
            if (Math.abs((e.clientX + camera.x) - currentStroke[currentStroke.length-2]) > strokeResolution || Math.abs((e.clientY + camera.y) - currentStroke[currentStroke.length-1]) > strokeResolution) {
        ...
        function drawCurrentStroke(e) {
            ...
            if (currentStroke.length > 2) {
                ...
                for (let j = 2; j < currentStroke.length-1; j+=2) {
                    ctx.lineTo(currentStroke[j] - camera.x, currentStroke[j+1] - camera.y);
                }
                ctx.stroke();
            } else {
                //draw dot if current stroke only a point
                ctx.beginPath();
                ctx.arc(currentStroke[0] - camera.x, currentStroke[1] - camera.y, 3, 0, Math.PI * 2, false);
                ctx.fill();
            }
        }
            ...
            for (let i = 0; i < strokes.length; i++) {
                if (strokes[i].length > 2) {
                    ...
                    for (let j = 2; j < strokes[i].length-1; j+=2) {
                        ctx.lineTo(strokes[i][j] - camera.x, strokes[i][j+1] - camera.y);
                    }
                    ctx.stroke();
                } else {
                    ctx.beginPath();
                    ctx.arc(strokes[i][0] - camera.x, strokes[i][1] - camera.y, 3, 0, Math.PI * 2, false);
                    ctx.fill();
                }
            }
        }
